...
// GET /api/users/all - Get all users for super admin dashboard
...
// GET /api/chat/summary - Get chat statistics for super admin dashboard
export const getChatSummary = async (req, res) => {
  try {
    ...
    const resolvedChats = await ChatSession.countDocuments({ status: 'resolved' });
    const unassignedChats = await ChatSession.countDocuments({ assignedTo: null });
    ...
    res.status(500).json({ message: "Failed to fetch chat summary" });
  }
};
